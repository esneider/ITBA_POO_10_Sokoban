...
public abstract class GameObject extends GameElement {
	 ...
	 * another GameTile. For example, is the next tile a Wall? We delegate that to
	 * the tile to say. Also, check for the current GameTile: Can I move out of this
	...
	public boolean canMove(Game game, Direction dir) {
		...
		GameTile fromTile = game.getTile(pos);
		if (!fromTile.canMoveFrom(dir)) {
			return false;
		}

		GameTile toTile = game.getTile(pos.getNeighbourPosition(dir));
	...
	public void move(Game state, Direction dir) {
		...
		GameTile fromTile = state.getTile(pos);
		GameTile toTile = state.getTile(toPos);
